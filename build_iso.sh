...
usage() {
    echo "build_iso.sh -i <INPUT_ISO> -o <OUTPUT_ISO>"
}
...
[[ -z $ISO_IN ]] && die "Specify source iso: -i <path/to/source.iso>"
[[ -f $ISO_IN ]] || die "Source iso not found: $ISO_IN"
[[ -z $ISO_OUT ]] && die "Specify iso destination path!"
[[ -f $GRUB_CFG ]] || die "Cannot find grub cfg: $GRUB_CFG"
...
if (! cp -v $GRUB_CFG $TMP_DIR/boot/grub) ; then
